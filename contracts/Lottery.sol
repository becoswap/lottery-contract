...
contract Lottery is Ownable, Initializable, Testable {
    ...
    // State variables 
    // Instance of Beco token (collateral currency for lotto)
    IERC20 internal beco_;
    // Storing of the NFT
    ...
    struct LottoInfo {
        ...
        uint256 prizePoolInBeco;    // The amount of beco for prize money
        uint256 costPerTicket;      // Cost per ticket in $beco
     ...
     * @param   _prizePoolInBeco The amount of Beco available to win in this 
     *          lottery.
        ...
        returns(uint256 lotteryId)
    {
        ...
        require(
            _prizePoolInBeco != 0 && _costPerTicket != 0,
    ...
    function withdrawBeco(uint256 _amount) external onlyOwner() {
        beco_.transfer(
            msg.sender, 
            _amount
        );
        ...
        notContract()
    {
        ...
        // Transfers the required beco to this contract
        beco_.transferFrom(
            msg.sender, 
            address(this), 
            costWithDiscount
    ...
    function claimReward(uint256 _lotteryId, uint256 _tokenId) external notContract() {
        ...
        allLotteries_[_lotteryId].prizePoolInBeco = allLotteries_[_lotteryId].prizePoolInBeco.sub(prizeAmount);
        // Transfering the user their winnings
        beco_.safeTransfer(address(msg.sender), prizeAmount);
    }
        ...
        notContract()
    {
        ...
        beco_.safeTransfer(address(msg.sender), totalPrize);
    }
     ...
     * @return  uint256: The prize amount in beco the user is entitled to 
     */
        ...
        returns(uint256) 
    {
        ...
        prize = allLotteries_[_lotteryId].prizePoolInBeco.mul(perOfPool);
