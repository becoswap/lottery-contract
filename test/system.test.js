...
describe("Lottery contract", function() {
    ...
    // Creating the instance and contract info for the beco token contract
    let becoInstance;
    ...
    beforeEach(async () => {
        ...
        becoInstance = await mock_erc20Contract.deploy(
            lotto.buy.beco,
        );
        ...
        // Making sure the lottery has some beco
        await becoInstance.mint(
            lotteryInstance.address,
    ...
    describe("Buying tickets tests", function() {
        ...
        it("Batch buying 1 tickets", async function() {
            ...
            await becoInstance.connect(owner).approve(
                lotteryInstance.address,
                price
            );
            // Batch buying tokens
         ...
         */
        it("Invalid beco transfer", async function() {
            // Getting the price to buy
            let price = await lotteryInstance.costToBuyTickets(
                1,
                10
    ...
    describe("Claiming tickets tests", function() {
        beforeEach( async () => {
            ...
            // Sending the buyer the needed amount of beco
            await becoInstance.connect(buyer).mint(
                buyer.address,
                prices[2]
            );
            // Approving lotto to spend cost
            await becoInstance.connect(buyer).approve(
                lotteryInstance.address,
                prices[2]
            );
            // Generating chosen numbers for buy
        ...
        it("Claiming winning numbers (4 (all) match)", async function() {
            ...
            // Sending the buyer the needed amount of beco
            await becoInstance.connect(owner).transfer(
                buyer.address,
                prices[2]
            );
            // Approving lotto to spend cost
            ...
            let buyerBecoBalanceBefore = await becoInstance.balanceOf(buyer.address);
            ...
            let buyerBecoBalanceAfter = await becoInstance.balanceOf(buyer.address);
            // Tests
            assert.equal(
                buyerBecoBalanceBefore.toString(),
                0,
                "Buyer has beco balance before claiming"
            );
            assert.equal(
                buyerBecoBalanceAfter.toString(),
    ...
    describe("Upgrade functionality tests", function() {
        ...
        it("Withdraw excess beco", async function() {
            let ownerBecoBalance = await becoInstance.balanceOf(owner.address);
            let lotteryBecoBalance = await becoInstance.balanceOf(lotteryInstance.address);
            await lotteryInstance.connect(owner).withdrawBeco(lotteryBecoBalance);
            let ownerBecoBalanceAfter = await becoInstance.balanceOf(owner.address);
            let lotteryBecoBalanceAfter = await becoInstance.balanceOf(lotteryInstance.address);
            // Tests
            assert.notEqual(
                ownerBecoBalance.toString(),
                ownerBecoBalanceAfter.toString(),
                "Owner beco balance did not change"
            );
            assert.notEqual(
                lotteryBecoBalance.toString(),
                lotteryBecoBalanceAfter.toString(),
        ...
        it("Invalid withdraw excess beco (non owner)", async function() {
            let lotteryBecoBalance = await becoInstance.balanceOf(lotteryInstance.address);
            await expect(
                lotteryInstance.connect(buyer).withdrawBeco(lotteryBecoBalance)
            ).to.be.revertedWith(lotto.errors.invalid_admin);
        });
    });
    ...
    describe("View function tests", function() {
        ...
        it("Get Lotto Info", async function() {
            ...
            assert.equal(
                lottoInfo.prizePoolInBeco.toString(),
